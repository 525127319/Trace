...
export default {
  ...
  created() {
    ...
    if (
      this.$store.state.modelSaleRatioDetailStTime &&
      !this.$store.state.modelSaleRatioDetailEdTime
    ) {
      self.startTime = this.$store.state.modelSaleRatioDetailStTime;
      let day = new Date();
      ...
      self.endTime = this.$store.state.modelSaleRatioDetailEdTime;
      let day = new Date();
      day.setDate(day.getDate() - 1);
    ...
    if (sessionStorage.getItem("asyncResult") && this.$route.query.asyncFlag) {
      ...
      this.$store.commit("SET_MODELSALEDETAIL_PARTS", this.newParts);
      ...
      this.$store.commit("SET_MODELSALEDETAIL_DATADETAIL", this.productList);
    } else {
      if(self.$store.state.saleDetailNoData){
        self.showNoDataICon =this.$store.state.saleDetailNoData;
      }
      if (self.$store.state.modelSaleRatioDetailType) {
        self.typeValue = this.$store.state.modelSaleRatioDetailType;
        self.activeIndex = this.$store.state.modelSaleRatioDetailIndex;
      } else {
        self.typeValue = "iPad";
        self.activeIndex = 0;
      }

      if (self.$store.state.modelSaleRatioDetailParts.length != 0) {
        self.newParts = this.$store.state.modelSaleRatioDetailParts;
        var hash = {};
  ...
  mounted() {
    if (this.$store.state.modelSaleRatioDetailData.length > 0) {
      this.productList = this.$store.state.modelSaleRatioDetailData;
    }
  },
  methods: {
    chooseStartTime() {
      this.$store.commit("SET_MODELSALEDETAIL_STARTTIME", this.startTime);
    },
    chooseEndTime() {
      this.$store.commit("SET_MODELSALEDETAIL_ENDTIME", this.endTime);
    },
    ...
    selecType(item, index) {
      ...
      this.typeValue = item;
      this.$store.commit("SET_MODELSALEDETAIL_TYPE", item);
      this.$store.commit("SET_MODELSALEDETAIL_INDEX", index);
      this.getInitModel();
    },
    // 去往选择机制页面
    goModelList() {
    ...
    getInitModel() {
      ...
      this.$axiosHttp
        ...
        .then(res => {
          ...
          this.$store.commit("SET_MODELSALEDETAIL_PARTS", self.newParts);
        });
    },
    handleSearch() {
      clearInterval(timer);
      ...
      this.$axiosHttp
        .axiosPost(
          "ks-newrep/InformationInfo/findInformationInfoShipments.api",
          params
        )
        .then(res => {
          // this.showDrag = true;
          ...
          if (code == 200) {
            if (data.length == 0) {
              ...
              this.$store.commit(
                "SET_MODELSALEDETAIL_DATADETAIL",
                this.productList
              );
            }
            this.$store.commit("SET_MODELSALEDETAIL_NODATA",this.showNoDataICon);
          } else if (code == -404) {
            this.showDrag = true;
          } else {
            this.errorMsg = msg;
    ...
    startAsyncTask: function() {
      ...
      const info = {
        ...
        traceApi:
          "http://ks-newrep/InformationInfo/findInformationInfoShipments.api",
        traceContext: {
