...
export default {
  ...
  created() {
     if(window.navigator.userAgent.indexOf('IOS')>-1||window.navigator.userAgent.slice(11)==='ANDROID-IAAPPCONTAINER'||window.navigator.userAgent.slice(11)==='PAD-IAAPPCONTAINER'){
      ...
      console.log(this.$route.query.initModel,'wip带的是什么参数');
      this.loadMore()
  },
  methods: {
    //单选
       ...
       if(idIndex >= 0){   
         ...
         this.partIds.splice(idIndex,1);
          let partsId =this.partIds;
           if(this.$route.query.path == "daysEntrepot"){
                this.$store.commit('SET_PARTS_DAYS', partsId);  
              }else if(this.$route.query.path == "dayEntrepot"){
                  this.$store.commit('SET_PARTS', partsId);
              }else if(this.$route.query.path =="compareStore"){
                 this.$store.commit('SET_COMPARESTORE_MODELS',partsId) 
              }           
        }else{
      ...
      this.$axiosHttp.axiosPost("ks-newrep/ReportTool/findModelByTypeName.api", data)
        .then(res => {
          ...
          if (code == 200 ) {
            ...
            if(this.$route.query.path == "daysEntrepot"){
               this.$store.state.dayParts.forEach(item => {
              ...
              if(this.$store.state.parts){
                  this.$store.state.parts.forEach(item => {
                    this.modelList.forEach(data=>{
                      if(item.modelId == data.modelId){
                        data.isShowIcon=!data.isShowIcon;
                        this.partIds.push(data);
              ...
              this.partIds.forEach(item =>{
                 ...
                 if(this.$store.state.compareStoreModels){
                  this.$store.state.compareStoreModels.forEach(item => {
                    this.modelList.forEach(data=>{
                      if(item.modelId == data.modelId){
                        data.isShowIcon=!data.isShowIcon;
                        this.partIds.push(data);
